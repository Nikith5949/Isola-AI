...
score st
    ...
    | m1 == 0                          = -1000.0
    | m2 == 0                          =  1000.0
    |otherwise                         =  fromIntegral $ ((m1 * 2) + z1) - ((m2 * 2) + z2)
    where m1 = (length $ genMoves st {pl1Turn = True })
          m2 = (length $ genMoves st {pl1Turn = False })
          r1 = (radius (pl1 st) 2)
          r2 = (radius (pl2 st) 2)
          z1 =  fromIntegral (length (r1)) - length((r1 \\(tiles st)))
          z2 =  fromIntegral (length (r2)) -length((r2 \\ (tiles st)))
